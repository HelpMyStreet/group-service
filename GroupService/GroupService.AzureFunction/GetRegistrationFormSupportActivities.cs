...
namespace GroupService.AzureFunction
{
    public class GetRegistrationFormSupportActivities
    {
        ...
        [FunctionName("GetRegistrationFormSupportActivities")]
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    return new ObjectResult(ResponseWrapper<GetRegistrationFormSupportActivitiesResponse, GroupServiceErrorCode>.CreateUnsuccessfulResponse(GroupServiceErrorCode.ValidationError, validationResults)) { StatusCode = 422 };
                }
            }
            catch (Exception ex)
            {
                _logger.LogErrorAndNotifyNewRelic($"Unhandled error in GetRegistrationFormSupportActivities", ex);
                return new ObjectResult(ResponseWrapper<GetRegistrationFormSupportActivitiesResponse, GroupServiceErrorCode>.CreateUnsuccessfulResponse(GroupServiceErrorCode.InternalServerError, "Internal Error")) { StatusCode = StatusCodes.Status500InternalServerError };
            }
        }
    }
}
